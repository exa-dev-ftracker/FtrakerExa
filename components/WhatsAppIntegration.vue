...
onMounted(async () => {
  try {
    ...
    // Fetch current user WhatsApp settings via axios plugin (cookie-based auth)
    const settingsRes = await (useNuxtApp().$axios as any).get(
      "/api/user/settings",
    );

    if (settingsRes?.data?.body) {
      phoneNumber.value = settingsRes.data.body.phone_number || "";
      chatbotEnabled.value = settingsRes.data.body.chatbot_enabled || false;
    }
  } catch (error: any) {
    // Settings not found or error - that's okay, user can set them
    console.debug(
      "WhatsApp settings not loaded:",
...
const savePhoneNumber = async () => {
  try {
    ...
    await (useNuxtApp().$axios as any).post("/api/user/phone", {
...
const toggleChatbot = async () => {
  try {
    ...
    await (useNuxtApp().$axios as any).patch("/api/user/chatbot", {
    ...
    toast.add({
      title: "Error",
      description:
        error?.data?.body?.message || "Failed to update chatbot status",
      color: "red",
    });
  } finally {
    isSaving.value = false;
  ...
  <div class="w-full max-w-2xl">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-600 dark:text-gray-400">
        Connect your WhatsApp number and enable chatbot automation to record
    ...
    <div v-else class="space-y-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="space-y-2">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            💡 Tip: Enter your WhatsApp number with country code (62 for
            Indonesia). It will be normalized to: 628123456789
          </p>
        </div>
      ...
      <!-- How to Use Section -->
      <div
        class="bg-blue-50 dark:bg-blue-900/20 rounded-lg shadow p-6 border border-blue-200 dark:border-blue-800"
      >
        ...
        <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
          ...
          <div class="flex gap-3">
            ...
            <span
              >Send a message to our WhatsApp bot in one of these formats:</span
            >
          </div>

          <div class="ml-8 space-y-2 font-mono text-xs">
            <div class="text-gray-600 dark:text-gray-400">
              <span class="text-green-600 dark:text-green-400">expense</span>
              50000 coffee
            </div>
            <div class="text-gray-600 dark:text-gray-400">
              <span class="text-green-600 dark:text-green-400">income</span>
              1000000 salary
            </div>
            <div class="text-gray-600 dark:text-gray-400">
              <span class="text-green-600 dark:text-green-400">e</span> 25000
              lunch
            </div>
          </div>
          ...
          <div class="flex gap-3">
            ...
            <span
              >Your transaction will be automatically recorded and added to your
              tracker</span
            >
          </div>
        </div>
      ...
      >
        ...
        <ul
          class="text-sm text-red-800 dark:text-red-300 space-y-1 list-disc list-inside"
        >
