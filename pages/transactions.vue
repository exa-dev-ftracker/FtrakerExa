...
const handleDeleteTransaction = async (id: string): Promise<void> => {
  ...
  try {
    ...
    toast.add({
      title: 'Error',
      description: error.message || 'An error occurred while trying to delete the transaction',
      color: 'red'
    })
  } finally {
...
watch(selectedView, async () => {
  await refresh()
  if ((error.value as any)?.statusCode === 401 && isHydrated.value) {
    toast.add({
      title: 'Unauthorized',
    ...
    <div class="py-6 sm:py-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p class="text-gray-500 dark:text-gray-400 mt-1">Manage and track your financial activities</p>
        </div>
      ...
      <div v-if="isHydrated" class="mb-8 mt-8 space-y-4">
        <div class="flex items-center overflow-x-auto pb-2 scrollbar-none gap-2 w-full border-b border-gray-200 dark:border-gray-800">
          <UButton 
      ...
      <div v-if="isHydrated">
        ...
        <!-- Empty State -->
        <div v-else-if="Object.keys(transactionByDate).length === 0" class="text-center py-16 px-4">
          <div class="inline-flex items-center justify-center p-4 bg-gray-100 dark:bg-gray-800 rounded-full mb-4">
            <UIcon name="i-heroicons-document-text" class="w-8 h-8 text-gray-500 dark:text-gray-400 block" />
          </div>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No transactions found</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">You don't have any transactions matching your current filters.</p>
          <UButton 
            @click="handleOpenModal(true)" 
            icon="i-heroicons-plus"
            color="primary"
        ...
        <div v-else class="space-y-8">
          <div v-for="(transactionOnDay, date) in transactionByDate" :key="date">
            <!-- Date Sticky Header -->
            <div class="sticky top-[60px] z-10 bg-white/90 dark:bg-gray-900/90 backdrop-blur border-b border-gray-200 dark:border-gray-800 py-3 mb-4 mt-2 transition-all px-4 rounded-t-lg">
      ...
      <div v-else class="flex justify-center items-center py-20">
        <UIcon name="i-heroicons-arrow-path" class="w-8 h-8 text-gray-400 animate-spin block" />
      </div>
    </div>
</template>
