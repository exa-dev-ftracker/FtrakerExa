...
const handleLoginSuccess = async (response: CredentialResponse) => {
  ...
  try {
    ...
    toast.add({ title: "Login Failed", description: err.message, color: "red" });
  } finally {
...
const handleLoginError = () => {
  toast.add({ title: "Login Failed", description: "Google Sign In failed", color: "red" });
};
...
const submit = async () => {
  ...
  try {
    const res = await (useNuxtApp().$axios as any).post("/api/auth/login", formData);
    const data = res.data || res;
      ...
      >
        ...
        <div class="p-6 sm:p-10 pb-0 text-center">
           <NuxtLink to="/" class="inline-flex items-center gap-3 mb-8 group">
              ...
              <span class="text-2xl font-black bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">FTraker</span>
           </NuxtLink>
           <Motion 
        ...
        <div class="p-6 sm:p-10 space-y-6">
           ...
           <Motion v-if="error" 
            ...
            >
              <UIcon name="i-heroicons-exclamation-triangle" class="w-5 h-5 text-rose-500 flex-shrink-0" />
              <p class="text-sm font-bold text-rose-600 dark:text-rose-400">{{ errorMessage }}</p>
           </Motion>
           ...
           <form @submit.prevent="submit" class="space-y-5">
              <Motion 
                ...
                class="space-y-2"
              >
                 <label class="text-sm font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest ml-1">Email Address</label>
                 <input 
              ...
              >
                 <div class="flex justify-between items-center ml-1">
                    <label class="text-sm font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">Password</label>
                    <a href="#" class="text-xs font-bold text-blue-600 hover:text-purple-600 transition-colors">Forgot Password?</a>
                 </div>
                 <input 
            ...
            <Motion 
               ...
               class="relative py-4"
             >
               <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-gray-200 dark:border-white/5"></div></div>
               <div class="relative flex justify-center text-xs uppercase font-black text-gray-400"><span class="bg-white dark:bg-[#0d1117] px-4">Or continue with</span></div>
            </Motion>
          ...
          >
            <p class="text-gray-500 dark:text-gray-400 font-bold">
               New here? 
               <NuxtLink to="/register" class="text-blue-600 hover:text-purple-600 transition-colors">Create an account</NuxtLink>
            </p>
         </Motion>
      </Motion>
    </div>
