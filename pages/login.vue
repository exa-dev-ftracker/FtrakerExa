...
const handleLoginSuccess = async (response: CredentialResponse) => {
    ...
    try {
        ...
        if (res.statusCode === 200 || res.statusCode === 201) {
            ...
            const message = res.statusCode === 201 ? 'Account created successfully!' : 'Logged in successfully!'
            toast.add({
                title: 'Success',
                description: message,
                color: 'green'
            ...
            toast.add({
                ...
                description: res.body?.message || 'Email not verified',
                color: 'red'
            })
            ...
            throw new Error(res.body?.message || 'An error occurred while trying to sign in')
        }
    } catch (err: any) {
        console.error('Google login error:', err)
        toast.add({
            title: 'Login Failed',
            description: err.data?.body?.message || err.message || 'An error occurred while trying to sign in',
            color: 'red'
        })
    } finally {
          ...
          <form @submit.prevent="submit" class="p-8">
            ...
            <div v-if="error" class="mb-6 p-4 bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700 rounded-xl">
              <div class="flex items-start gap-3">
                ...
                <div>
                  <p class="text-red-700 dark:text-red-200 font-semibold">Login Failed</p>
