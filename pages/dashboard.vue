...
const expenseTotal = computed(() => {
  return (data.value?.body?.current || []).reduce((sum: number, t: Transaction) => {
    ...
    console.log(`Transaction: ${t.description}, Type: ${type}, Amount: ${t.amount}`)
...
watch(error, (newError) => {
  if (newError && isHydrated.value) {
    toast.add({
      title: 'Error',
      description: (newError as any)?.data?.message || 'An error occurred while loading dashboard',
      color: 'red'
    })
  }
})
...
<template>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      ...
      <!-- Welcome Hero -->
      <section class="mb-12 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white rounded-2xl shadow-xl p-8 md:p-12">
      ...
      <section v-if="isHydrated && !loading" class="mb-12">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <!-- Balance Card -->
          <div class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500 via-purple-600 to-purple-700 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 text-white p-8">
            ...
            <div class="relative">
              <div class="flex items-center justify-between mb-6">
                <p class="text-sm font-semibold opacity-90 uppercase">Current Balance</p>
                <div class="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
                  <i class="i-material-symbols-account-balance-wallet text-2xl"></i>
                </div>
              </div>
              <p class="text-4xl font-black mb-2">{{ currency(balance) }}</p>
              <p class="text-sm opacity-80">{{ balance > 0 ? '✅ Positive' : '⚠️ Negative' }}</p>
            </div>
          </div>

          <!-- Income Card -->
          <div class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-400 via-green-500 to-emerald-600 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 text-white p-8">
            <div class="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            <div class="relative">
              <div class="flex items-center justify-between mb-6">
                <p class="text-sm font-semibold opacity-90 uppercase">Total Income</p>
                <div class="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
          ...
          <!-- Expense Card -->
          <div class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-400 via-red-500 to-rose-600 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 text-white p-8">
            <div class="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            <div class="relative">
              <div class="flex items-center justify-between mb-6">
                <p class="text-sm font-semibold opacity-90 uppercase">Total Expense</p>
                <div class="p-3 bg-white/20 rounded-lg backdrop-blur-sm">
                  <i class="i-material-symbols-trending-down-outline text-2xl"></i>
                </div>
              </div>
              <p class="text-4xl font-black mb-2">-{{ currency(expenseTotal) }}</p>
              <p class="text-sm opacity-80">📊 This {{ selectedView }}</p>
            </div>
          </div>
        </div>
      ...
      <!-- Content Grid -->
      <div v-if="isHydrated && !loading" class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <!-- Recent Transactions (Main) -->
        <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8">
          <div class="flex items-center justify-between mb-6">
          ...
          <div v-if="recentTransactions.length > 0" class="space-y-3">
            ...
            >
              ...
              <p :class="[
                ...
                {{ transaction.type.toLowerCase() === 'income' ? '+' : '-' }}{{ currency(transaction.amount) }}
              </p>
            </div>
          </div>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8">
            <div class="flex items-center justify-between mb-4">
              <p class="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase">Transactions</p>
              <i class="i-material-symbols-counter text-2xl text-purple-600"></i>
            </div>
            <p class="text-3xl font-black text-gray-900 dark:text-white">{{ transactionCount }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">This {{ selectedView }}</p>
          </div>

          <!-- Top Expenses -->
          <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <i class="i-material-symbols-trending-down text-red-600 text-xl"></i>
              Top Expenses
            </h3>
            <div v-if="topExpenses.length > 0" class="space-y-3">
              ...
              >
                <p class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ desc }}</p>
                <p class="text-sm font-bold text-red-600 flex-shrink-0">{{ currency(amount) }}</p>
              </div>
            </div>
          ...
          <!-- Quick Actions -->
          <div class="bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/20 dark:to-purple-900/20 rounded-2xl p-8">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
...
.min-h-screen > section, .min-h-screen > div:not(.absolute) {
